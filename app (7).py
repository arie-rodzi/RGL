...
gloss = pd.DataFrame({
    ...
    "Scientific name": [
        ...
        "River class/status label (0=Clean/Safe, 1=Polluted) — ideally derived from WQI class"
    ]
})
st.dataframe(gloss, use_container_width=True, hide_index=True)

st.info("**Comparing with WQI** — If your dataset's `RIVERSTATUS` was assigned from the Malaysian WQI classes, the app will automatically compare predicted status vs. WQI status (your `RIVERSTATUS`). Upload your WQI-labelled data and see the confusion table/metrics in either Manual or Train modes. If you want the app to compute WQI from raw readings, tell me which exact DOE WQI formula version/weights to use and I'll wire it in.")
...
def format_display(df: pd.DataFrame) -> pd.DataFrame:
    # Pretty labels for UI (download keeps simple names)
    ...
    with tab2:
        ...
        if file:
            ...
            try:
                ...
                st.error("Failed to read Excel file. Ensure 'openpyxl' is installed and the file is a valid .xlsx.")
                st.exception(_e_xl)
                df = pd.DataFrame()
